...
const cookieCache = {
   ...
   *                类型为number时：单位默认为‘天’
   *                类型为string时：支持单位 D（天） | H（时） | M（分） | S（秒），例子：2D、15D、1.55D、24H、60M、60S
   * @returns
   */
...
const sessionCache = {
  ...
  get(key: string): string | null {
    ...
    return JSON.parse(JSON.stringify(uni.getStorage({ key: key })));
  },
  /**
   * 移除sessionStorage
   * @param key
...
export enum CacheEnum {
  /** 登录通行证：用户信息、token等 */
  DTM_PASSPORT = "dtm_passport",
  /** 当前语言环境 */
  DTM_LANGUAGE = "dtm_language",
  /** 高级查询保存的查询条件key */
  DTM_ADVANCED_USE_KEY = "dtm_advanced_use_key",
}
